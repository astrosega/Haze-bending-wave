...
pro Figure2
  ...
  for star_i=0, noccs-1 do begin
    ...
    ;(Radio occultations) the best fitted amplitude differed by this one by a factor of 4 while in SCL (optical) it differed by 1.15    ;w = (d*1000*Omega)^2/(sigma*G)
    ;W = 58.5
    ...
    if keyword_set(plots) then begin
      ...
      modelplot   = plot(r-rv, optd, xrange=[-200.,50.], yrange=[0.,5.], thick=2, name="SCL",linestyle=0, dimensions = [1366,768], buffer= 0)

      graphic4   = oplotmimas_fig212(radius,data,star='('+stars_format+')',tao=tao, title=1, uperror=taoplus,downerror=taominus,modelplot=modelplot);,scl=1);, error=taosigma) ;, angles = [B, phi], phase = phase_print)
    ...
    if keyword_set(joshplot) then begin
      ...
      graphic  = oplotmimas_fig212(radius,data,star='('+stars_format+')',tao=tao, title=1,uperror=taoplus,downerror=taominus,layout=1,modelplot=optional2, optional=optional1);, error=taosigma) ;, angles = [B, phi], phase = phase_print)
