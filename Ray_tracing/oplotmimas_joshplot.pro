function oplotmimas_joshplot, radius, data, nsum = nsum, rebin=rebin, star=star, title=title, Tao=taosum, ANGLES=angles, PHASE=phase, uperror=uperror,downerror=downerror, scl=SCL,layout=layout,modelplot=modelplot,optional=optional
;Version of oplotmimas made specific for figure3.pro
  ...
  if KEYWORD_SET(title) then begin
    ...
    if keyword_set(layout) then begin
      ...
      graphic = plot(radius[r[0]:r[1]] -rv, taosum[r[0]:r[1]], yrange=[0,5], xtitle='Distance from resonance [km]', ytitle='Optical Depth', color='red', font_size = 24, $
        XTICKFONT_SIZE=24, YTICKFONT_SIZE=24, Name = 'Data',overplot=1)

      ; graphic.title.font_size = 24
    endelse

    if keyword_set(layout) then leg  = legend(position=[.66,.93], font_size = 20,target=[modelplot,optional,graphic]) else leg  = legend(position=[.8,.8], font_size = 20,target=[modelplot,graphic])

    if KEYWORD_SET(angles) then begin

      junk =  text(0.2, 0.80, 'B   =' + string(angles[0]*180./!dpi))
      junk =  text(0.2, 0.75, 'Phi =' + string(angles[1]*180./!dpi))
