...
pro Figure12;, P0, name=name, plots = plots, isotro = isotro, highb = HighB, fit_rv = fit_rv, create_haze = create_haze
  ;tic
  ;fit_rv = 0
  fit_phase = 1
  create_haze = 1
  ...
  suba = [where(strmatch(stars, 'GamPeg032*') eq 1)]
...
star_i=0
  ...
  ;  if star_i eq 6 then sm=20
   ...
   junk = func_shugampeg() ;This computes the raytracing for a normal occultation according to the SCL theory to overplot against the model. In the orignal code, this vector was produced one and they recalled, but for the purpose of having this code be self-contained, it is now computed within the code.
        rscl = junk[*,0]
        optd = junk[*,1]
...
graphic5   = plot(riso-rv, optdiso,sm, xrange=[-200.,50.], yrange=[0.,max([max(optd),max(tao), max(optdiso)])+.6], thick=3, name="New Model",overplot=1,linestyle=0, dimensions = [1366,768])
   ...
   ; leg  = legend(position=[.9,.8], font_size = 20,target=[aniso,graphic4])
      leg  = legend(position=[.9,.8], font_size = 20,target=[iso,aniso,graphic4])
    ...
    graphic5.save, stars_format+'six.png'
