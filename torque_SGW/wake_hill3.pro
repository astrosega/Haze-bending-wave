...
tic
  ...
  for i=2d, time do begin
    ...
    stub = hillcol4d(b, l,w,h,s,rho,rho_mean, x[0], x[1], x[2], y[0], y[1], y[2], z[0], z[1], z[2], omegax[i-1],omegay[i-1],omegaz[i-1], ind=i, phis=phis, dhdot = dhdot, thetays = thetays, thetaxs=thetaxs, intl = IntL,intw=intw,inth=inth,vzrecord=vzrecord)
    ...
    stub = hillcol4d(b, l,w,h,s,rho,rho_mean, xhalf[0], xhalf[1], xhalf[2], yhalf[0], yhalf[1], yhalf[2], zhalf[0], zhalf[1], zhalf[2], omegaxhalf, omegayhalf, omegazhalf, ind=i, phis=phis, dhdot = dhdot, thetays = thetays, thetaxs=thetaxs, intl = IntL,intw=intw,inth=inth,vzrecord=vzrecord)
  ...
  cgplot,seconds*(2*!dpi/omega)^(-1),   x2s ,color=cgcolor('blue'),charsize=2.8, title='Time evolution of self-gravity wakes orientation, vertical velocity relative to the Ring (L40)', xtitle='Time (Orbital Periods)', ytitle='Degrees',yrange=[-1,1],xrange=[0,Ts];,yrange=[-2,2]
...
;  cgAxis, YAxis=1, YRange=[0, 0.01],charsize=2.8, /Save, ytitle='Relative speed [m/s]'
  ...
  write_png,'seeThefolder.png',TVRD(/TRUE)
  SAVE, /VARIABLES, FILENAME = 'seethefolderdt5.sav'
