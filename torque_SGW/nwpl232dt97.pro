...
Pro nwpl232dt97, seconds, omega, thetaxs, thetays, phis, deltav, deltavk, vkdr, omegax, omegay, omegaz, w, l, h, x0s, y0s, x2s, x1s, z1s, z2s, dhdot,couplex,coupley,couplez,gravx,gravy,gravz,kepx,kepy,kepz,tidalx,tidaly,tidalz
  tic
  ;Dynamics of a self-gravity wake within a bending wave. This code computes the different torques and advances the wake's orientation with a midpoint method. "nwp" stands for no wake potential, because this run does not include the potential generated by other wakes.
  ...
  ;dhdot                  --> time derivative of the local slope of the wave over time.
  
  ;Change log
  ;08-19-2022  --> created by Daniel Sega
 ...
 ; dt = 1425d
  ...
  couplex = make_array(time + 1d, /DOUBLE)
  coupley = make_array(time + 1d, /DOUBLE)
  couplez = make_array(time + 1d, /DOUBLE)
  
  tidalx = make_array(time + 1d, /DOUBLE)
  tidaly = make_array(time + 1d, /DOUBLE)
  tidalz = make_array(time + 1d, /DOUBLE)


  kepx = make_array(time + 1d, /DOUBLE)
  kepy = make_array(time + 1d, /DOUBLE)
  kepz = make_array(time + 1d, /DOUBLE)
  
  gravx = make_array(time + 1d, /DOUBLE)
  gravy = make_array(time + 1d, /DOUBLE)
  gravz = make_array(time + 1d, /DOUBLE)

  ;wakex = make_array(time + 1d, /DOUBLE)
  ;kepy = make_array(time + 1d, /DOUBLE)
  ;kepz = make_array(time + 1d, /DOUBLE)
  ...
  for i=2d, time do begin
    ...
    ;print,torquezhalf
    ;       print,omegay[i-1]
;plot,seconds*(2*!dpi/omega)^(-1),sqrt(kepx^2+kepy^2+kepz^2),xrange=[0,0.2]
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;endif
    ...
    kepx[1] = -(2 * muminus*(rho*h*w*l^3/24.) * yhalf[0]*zhalf[0]  - 2 * muplus*(rho*h*w*l^3/24.) * yhalf[1]*zhalf[1] )
     kepy[1] =-(- 2 * muminus*(rho*h*l*w^3/24.) * xhalf[0]*zhalf[0]  + 2 * muplus*(rho*h*l*w^3/24.) * xhalf[1]*zhalf[1])
      kepz[1] = -(- 2 * muminus*(rho*h*w*l^3/24.) * yhalf[0]*xhalf[0]  + 2 * muplus*(rho*h*w*l^3/24.) * xhalf[1]*yhalf[1] )
      tidalx[1] =   2 * muminus*(rho*h*w*l^3/24.) * yhalf[0]*zhalf[0]  - 2 * muplus*(rho*h*w*l^3/24.) * yhalf[1]*zhalf[1]
      tidaly[1] = - 2 * muminus*(rho*h*l*w^3/24.) * xhalf[0]*zhalf[0]  + 2 * muplus*(rho*h*l*w^3/24.) * xhalf[1]*zhalf[1]
      tidalz[1] = - 2 * muminus*(rho*h*w*l^3/24.) * yhalf[0]*xhalf[0]  + 2 * muplus*(rho*h*w*l^3/24.) * xhalf[1]*yhalf[1]
    endif

    
    kepx[i] = (dragx/dt)*(Ixx)
    kepy[i] = (dragy/dt)*(Iyy) ;make_array(time + 1d, /DOUBLE)
    kepz[i] = (dragz/dt)*(Izz) ;make_array(time + 1d, /DOUBLE)
    
    tidalx[i] =   2 * muminus*(rho*h*w*l^3/24.) * yhalf[0]*zhalf[0]  - 2 * muplus*(rho*h*w*l^3/24.) * yhalf[1]*zhalf[1] 
    tidaly[i] = - 2 * muminus*(rho*h*l*w^3/24.) * xhalf[0]*zhalf[0]  + 2 * muplus*(rho*h*l*w^3/24.) * xhalf[1]*zhalf[1]
    tidalz[i] = - 2 * muminus*(rho*h*w*l^3/24.) * yhalf[0]*xhalf[0]  + 2 * muplus*(rho*h*w*l^3/24.) * xhalf[1]*yhalf[1] 


    couplex[i] = (inertiax/dt)*(Ixx)
    coupley[i] = (inertiay/dt)*(Iyy)
    couplez[i] = (inertiaz/dt)*(Izz)
   ; if i gt 502 then begin
   ;   print,'stop'
   ; endif

    gravy[i] = -2*dhdotdothalf*(rho*h*l*w*w*w/24.) * zhalf[2] * xhalf[0]
    gravx[i] =  2*dhdotdothalf*(rho*h*w*l*l*l/24.) * zhalf[2] * yhalf[0]
    gravz[i] = -2*dhdotdothalf*(rho*h*w*l*l*l/24.) * xhalf[2] * yhalf[0]
...
;restore, 'nwpl236dt97wtorquesfix.sav'
...
ts=3
  ...
  cgplot,seconds*(2*!dpi/omega)^(-1),   alog10(sqrt((kepx)^2+(kepy)^2+(kepz)^2)) ,color=cgcolor('blue'),charsize=2.8, xtitle='Time (Orbital Periods)', ytitle='Angular acceleraion [1/s^2]',xrange=[0,3],nsum=3;,yrange=[-2,2]
  cgoplot,seconds*(2*!dpi/omega)^(-1),  alog10(sqrt((gravx)^2+(gravy)^2+(gravz)^2)), color = cgcolor('red')
  cgoplot,seconds*(2*!dpi/omega)^(-1),  alog10(sqrt(tidalx^2+tidaly^2+tidalz^2)),color= cgcolor('green')
  cgoplot,seconds*(2*!dpi/omega)^(-1),  alog10(sqrt(couplex^2+coupley^2+couplez^2)),color= cgcolor('magenta'),nsum=5
...
;  write_png,'seeThefolder.png',TVRD(/TRUE)
  ...
  SAVE, seconds, omega, thetaxs, thetays, phis, deltav, deltavk, vkdr, omegax, omegay, omegaz, w, l, h, x0s, y0s, x2s, x1s, z1s, z2s, dhdot,couplex,coupley,couplez,gravx,gravy,gravz,kepx,kepy,kepz,tidalx,tidaly,tidalz ,FILENAME = 'nwpl236dt97.sav'
