...
Pro nwpl_stability
  ...
  if flag then begin
    ...
    thetax[0] =  .025;-slope*sin(phi) ; omegax[1]*dt
    thetax[1] =  .025;-slope*sin(phi) ; omegax[1]*dt
  ...
  Rotation = [[1, 0, 0], [0, Cos(thetax[1]), Sin(thetax[1])], [0, -Sin(thetax[1]), Cos(thetax[1])]]
  ...
  for i=2d, time do begin
    ...
    torqueyhalf  = -2*dhdotdot*(rho*h*l*w^3/24.) * z[2]*x[0] /(Iyy) * dt/2. - 2 * muminus*(rho*h*l*w^3/24.) * x[0]*z[0] / (Iyy) * dt/2. + 2 * muplus*(rho*h*l*w^3/24.) * (x[1]*z[1] + x[2]*z[2]) / (Iyy) * dt/2. + 4*(!dpi)*G*(sigma)*(k)* H * (rho*h*l*w*w*w/24.) * Sin(Phi) * Cos(Phi) * (z[1]*x[0] + z[0]*x[1]) / (Iyy) * dt/2.
    torquexhalf  =  2*dhdotdot*(rho*h*w*l^3/24.) * z[2]*y[0] /(Ixx) * dt/2. + 2 * muminus*(rho*h*w*l^3/24.) * y[0]*z[0] / (Ixx) * dt/2. - 2 * muplus*(rho*h*w*l^3/24.) * (y[1]*z[1] - y[2]*z[2]) / (Ixx) * dt/2. - 4*(!dpi)*G*(sigma)*(k)* H * (rho*h*w*l*l*lo/24.) * Sin(Phi) * Cos(Phi) * (z[1]*y[0] + z[0]*y[1]) / (Ixx) * dt/2.
    torquezhalf  = -2*dhdotdot*(rho*h*w*l^3/24.) * x[2]*y[0] /(Izz) * dt/2. - 2 * muminus*(rho*h*w*l^3/24.) * y[0]*x[0] / (Izz) * dt/2. + 2 * muplus*(rho*h*w*l^3/24.) * (x[1]*y[1] + y[2]*x[2])/ (Izz) * dt/2. + 4*(!dpi)*G*(sigma)*(k)* H * (rho*h*w*l*l*lo/24.) * Sin(Phi) * Cos(Phi) * (x[1]*y[0] + x[0]*y[1]) / (Izz) * dt/2.
    ...
    torquey = - 2*dhdotdothalf*(rho*h*l*w*w*w/24.) * zhalf[2] * xhalf[0]/(Iyy) * dt - 2 * muminus*(rho*h*l*w^3/24.) * xhalf[0]*zhalf[0] / (Iyy) * dt + 2 * muplus*(rho*h*l*w^3/24.) * (xhalf[1]*zhalf[1]+ xhalf[2]*zhalf[2]) / (Iyy) * dt + 4*(!dpi)*G*(sigma)*(k)* H * (rho*h*lo*w*w*w/24.) * Sin(Phi) * Cos(Phi) * (zhalf[1]*xhalf[0] + zhalf[0]*xhalf[1]) / (Iyy) * dt
    torquex =   2*dhdotdothalf*(rho*h*w*l*l*l/24.) * zhalf[2] * yhalf[0]/(Ixx) * dt + 2 * muminus*(rho*h*w*l^3/24.) * yhalf[0]*zhalf[0] / (Ixx) * dt - 2 * muplus*(rho*h*w*l^3/24.) * (yhalf[1]*zhalf[1]- yhalf[2]*zhalf[2]) / (Ixx) * dt - 4*(!dpi)*G*(sigma)*(k)* H * (rho*h*w*lo*l*l/24.) * Sin(Phi) * Cos(Phi) * (zhalf[1]*yhalf[0] + zhalf[0]*yhalf[1]) / (Ixx) * dt
    torquez = - 2*dhdotdothalf*(rho*h*w*l*l*l/24.) * xhalf[2] * yhalf[0]/(Izz) * dt - 2 * muminus*(rho*h*w*l^3/24.) * yhalf[0]*xhalf[0] / (Izz) * dt + 2 * muplus*(rho*h*w*l^3/24.) * (xhalf[1]*yhalf[1]+ yhalf[2]*xhalf[2]) / (Izz) * dt + 4*(!dpi)*G*(sigma)*(k)* H * (rho*h*w*lo*l*l/24.) * Sin(Phi) * Cos(Phi) * (xhalf[1]*yhalf[0] + xhalf[0]*yhalf[1]) / (Izz) * dt
  ...
  ;cgplot,seconds*(2*!dpi/omega)^(-1),   y0s ,color=cgcolor('blue'),charsize=2.8, xtitle='Time (Orbital Periods)', ytitle='Angular acceleraion [1/s^2]',xrange=[0,2];,yrange=[-2,2]
  ;cgoplot,seconds*(2*!dpi/omega)^(-1),  y1s, color = cgcolor('red')
  ;cgoplot,seconds*(2*!dpi/omega)^(-1),  y2s,color= cgcolor('green')

ts=2

  cgplot,seconds*(2*!dpi/omega)^(-1),     thetaxs*180/!dpi ,color=cgcolor('blue'),charsize=2.8, title='Time evolution of self-gravity wakes orientation, vertical velocity relative to the Ring (L40)', xtitle='Time (Orbital Periods)', ytitle='Degrees',yrange=[-20,60],xrange=[0,Ts];,yrange=[-2,2]
  ...
  cgLegend, Color=['Blue','Red', 'Green'], Location=[0.72, 0.92],  Titles=['!4h!Xx', '!4h!Xy','!4h!Xz'], Length=0.075, /Box, VSpace=2.75,/Background, bg_color='white',charsize=2.2
  ...
  cgplot,seconds*(2*!dpi/omega)^(-1),  deltavk*100, nsum=1, xtitle='Time (Orbital Periods)',  ytitle='Relative speed [cm/s]',charsize=2.8,xrange=[0,Ts],yrange=[1,1];
  ...
  write_png,'stabilityWakePotential2.png',TVRD(/TRUE)
  ...
  SAVE, seconds, omega, thetaxs, thetays, phis, deltav, deltavk, vkdr, omegax, omegay, omegaz, w, l, h, x0s, y0s, x2s, x1s, z1s, z2s, dhdot, FILENAME = 'nwpl236dt97ultimate2.sav'
